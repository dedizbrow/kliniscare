$modalManagePembayaranHutang	= "#modal-manage-pembayaran-hutang";
$modalDetail					= "#modal-detail"
$tableData		= $("#dataHutang").DataTable({

    serverSide: true,
    ordering: true,
    pageLength: "50",
    ajax: {
        url: base_url('farmasi/hutang/load-dt'),
        type: 'POST',
        headers: {
...
$(document)
	...
	.on("click",".link-bayar-hutang",function(){
		http_request('farmasi/hutang/search__','GET',{id: $(this).data('id')})
		.done(function(result){
			var data=result.data;
			var sisa=result.sisa;
			var total_dibayar=result.total_dibayar;
			$modal_id=$modalManagePembayaranHutang;
	...
	.on("submit","form[name='form-manage-pembayaran-hutang']",function(e){
		e.preventDefault();
		$("#save-pembayaran-hutang").attr('disabled', 'disabled');
		$form=$(this).closest('form');
		var data = $form.serialize();
		http_request('farmasi/hutang/save__','POST',data)
		.done(function(res){
			$($modalManagePembayaranHutang).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-pembayaran-hutang").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-pembayaran-hutang").removeAttr('disabled');
			})
	...
	.on("click",".link-detail-hutang",function(){
		http_request('farmasi/hutang/search_detail_bayar_hutang','GET',{id: $(this).data('id')})
		.done(function (result) {
