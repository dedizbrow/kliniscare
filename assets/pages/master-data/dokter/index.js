...
$modalManageSpesialisasi = "#modal-manage-spesialisasi";
$modalManageImport = "#modal-manage-import";
$tableData = $("#dataDokter").DataTable({
    serverSide: true,
    ordering: true,
    pageLength: "50",
    ajax: {
        url: base_url('master-data/dokter/load-dt'),
        type: 'POST',
        headers: {
    ...
    columnDefs: [
		...
		{ targets: [6], width: '115px',className: 'text-center' },
...
$(document)
	.ready(function () {
		getActiveLang('master-data/dokter');
	})
	...
	.on("click",".link-edit-dokter",function(){
		http_request('master-data/dokter/search__','GET',{id: $(this).data('id')})
		.done(function(result){
			var data=result.data;
			$modal_id=$modalManageDokter;
			$modal_body = $($modal_id).find('.modal-body');

			$('#spesialisasi').html('<option value="'+data.idSpesialisasi+'">'+data.namaSpesialisasi+'</option>').trigger('change');

			$.each(data, function (key, val) {
				if(key=='status'){
					$modal_body.find("[name='" + key + "'][value='" + val + "']").attr("checked", true);
				}
				else{
	...
	.on("submit","form[name='form-manage-dokter']",function(e){
		...
		http_request('master-data/dokter/save__','POST',data)
		.done(function(res){
			$($modalManageDokter).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-dokter").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-dokter").removeAttr('disabled');
			})
	...
	.on("click",".link-delete-dokter",function(){
		...
		bootbox.confirm({
			...
			callback: function (result) {
						if(result){
							http_request('master-data/dokter/delete__/'+id,'DELETE',{})
							.done(function(res){
									Msg.success(res.message);
	...
	.on("click", ".add-spesialisasi", function () {
		$($modalManageDokter).modal('hide');
		$modal_id=$modalManageSpesialisasi;
		$modal_body=$($modal_id).find('.modal-body');
	...
	.on("submit","form[name='form-manage-spesialisasi']",function(t){
		t.preventDefault();
		$("#save-spesialisasi").attr('disabled', 'disabled');
		$form=$(this).closest('form');
		var data = $form.serialize();
		http_request('master-data/dokter/save_spesialisasi','POST',data)
		.done(function(res){
			$($modalManageSpesialisasi).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-spesialisai").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-spesialisai").removeAttr('disabled');
			})
			.always(function () {
			$("#save-spesialisasi").removeAttr('disabled');
			$($modalManageDokter).modal('show');
		})
	})
// control import
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				...
				$.ajax({
					url: base_url('master-data/dokter/import_'),
					type: 'POST',
					data: formData,
					async: false,
					cache: false,
					...
					success: function (response) {
						...
						if (response.duplicate && response.duplicate.length>0) {
							$(".row-error .content").html('');
							$(".row-error .title").html('NIP sudah terdaftar (Duplicate):');
		...
		$("#spesialisasi").select2({
			...
			tags: true,
			ajax: { 
			url: base_url('master-data/dokter/select2_'),
			type: "GET",
			dataType: 'json',
