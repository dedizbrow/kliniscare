$modalManageKaryawan = "#modal-manage-karyawan";
$modalManageImport = "#modal-manage-import";
$modalManageBidang = "#modal-manage-bidang";
$tableData = $("#dataKaryawan").DataTable({
    serverSide: true,
    ordering: true,
    pageLength: "50",
    ajax: {
        url: base_url('master-data/karyawan/load-dt'),
        type: 'POST',
        headers: {
    ...
    columnDefs: [
		...
		{ targets: [7], width: '15px',className: 'text-center', orderable: false },
    ],
...
$(document)
	.ready(function () {
		getActiveLang('master-data/karyawan');
	})
	.on("click", ".add-karyawan", function () {
		$modal_id=$modalManageKaryawan;
	...
	.on("click",".link-edit-karyawan",function(){
		http_request('master-data/karyawan/search__','GET',{id: $(this).data('id')})
		.done(function(result){
			var data=result.data;
			$modal_id=$modalManageKaryawan;
			$modal_body = $($modal_id).find('.modal-body');

			$('#bidang').html('<option value="'+data.id+'">'+data.nama+'</option>').trigger('change');
	...
	.on("click", ".add-bidang", function () {
		$($modalManageKaryawan).modal('hide');
		$modal_id=$modalManageBidang;
		$modal_body=$($modal_id).find('.modal-body');
	...
	.on("submit","form[name='form-manage-bidang']",function(t){
		t.preventDefault();
		$("#save-bidang").attr('disabled', 'disabled');
		$form=$(this).closest('form');
		var data = $form.serialize();
		http_request('master-data/karyawan/save_bidang','POST',data)
		.done(function(res){
			$($modalManageBidang).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-bidang").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-bidang").removeAttr('disabled');
			})
			.always(function () {
			$("#save-bidang").removeAttr('disabled');
			$($modalManageKaryawan).modal('show');
		})
	})
	.on("submit","form[name='form-manage-karyawan']",function(e){
		e.preventDefault();
		$("#save-karyawan").attr('disabled', 'disabled');
		$form=$(this).closest('form');
		var data = $form.serialize();
		http_request('master-data/karyawan/save__','POST',data)
		.done(function(res){
			$($modalManageKaryawan).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-karyawan").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-karyawan").removeAttr('disabled');
			})
	...
	.on("click",".link-delete-karyawan",function(){
		hideMsg();
		var id=$(this).data('id');
		$that=$(this);
		bootbox.confirm({
			...
			callback: function (result) {
						if(result){
							http_request('master-data/karyawan/delete__/'+id,'DELETE',{})
							.done(function(res){
									Msg.success(res.message);
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				...
				$.ajax({
					url: base_url('master-data/karyawan/import_'),
					type: 'POST',
					data: formData,
					async: false,
					cache: false,
					...
					success: function (response) {
						...
						if (response.duplicate && response.duplicate.length>0) {
							$(".row-error .content").html('');
							$(".row-error .title").html('Karyawan sudah terdaftar (Duplicate):');
		...
		$("#bidang").select2({
            ...
            tags: true,
            ajax: { 
            url: base_url('master-data/karyawan/select2_bidang'),
			type: "GET",
            dataType: 'json',
            delay: 250,
