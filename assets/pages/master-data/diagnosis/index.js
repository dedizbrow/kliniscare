$modalManageDiagnosis = "#modal-manage-diagnosis";
$modalManageImport = "#modal-manage-import";
$tableData = $("#dataDiagnosis").DataTable({
    serverSide: true,
    ordering: true,
    pageLength: "50",
    ajax: {
        url: base_url('master-data/diagnosis/load-dt'),
        type: 'POST',
        headers: {
    ...
    columnDefs: [
		...
		{ targets: [2], width: '220px',className: 'text-left' },
		{ targets: [4], width: '15px',className: 'text-center', orderable: false },
    ],
...
$(document)
	.ready(function () {
		getActiveLang('master-data/diagnosis');
	})
	.on("click", ".add-diagnosis", function () {
		$modal_id=$modalManageDiagnosis;
	...
	.on("click",".link-edit-diagnosis",function(){
		http_request('master-data/diagnosis/search__','GET',{id: $(this).data('id')})
	...
	.on("submit","form[name='form-manage-diagnosis']",function(e){
		e.preventDefault();
		$("#save-diagnosis").attr('disabled', 'disabled');
		$form=$(this).closest('form');
		var data = $form.serialize();
		http_request('master-data/diagnosis/save__','POST',data)
		.done(function(res){
			$($modalManageDiagnosis).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-diagnosis").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-diagnosis").removeAttr('disabled');
			})
	...
	.on("click",".link-delete-diagnosis",function(){
		hideMsg();
		var id=$(this).data('id');
		$that=$(this);
		bootbox.confirm({
			...
			callback: function (result) {
						if(result){
							http_request('master-data/diagnosis/delete__/'+id,'DELETE',{})
							.done(function(res){
									Msg.success(res.message);
...
.on("click", ".import__", function () {
	...
	modal_body.find("input:text").val("");
	modal_body.find("textarea[name='deskripsi']").val("");
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				...
				$.ajax({
					url: base_url('master-data/diagnosis/import_'),
					type: 'POST',
					data: formData,
					async: false,
					cache: false,
					...
					success: function (response) {
						...
						if (response.duplicate && response.duplicate.length>0) {
							$(".row-error .content").html('');
							$(".row-error .title").html('Diagnosa sudah terdaftar (Duplicate):');
