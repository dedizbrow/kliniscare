$modalManagePoli = "#modal-manage-poli";
$modalManageImport = "#modal-manage-import";
$tableData = $("#dataPoli").DataTable({
    serverSide: true,
    ordering: true,
    pageLength: "50",
    ajax: {
        url: base_url('master-data/poli/load-dt'),
        type: 'POST',
        headers: {
...
$(document)
	.ready(function () {
		getActiveLang('master-data/poli');
	})
	.on("click", ".add-poli", function () {
		$modal_id=$modalManagePoli;
	...
	.on("click",".link-edit-poli",function(){
		http_request('master-data/poli/search__','GET',{id: $(this).data('id')})
	...
	.on("submit","form[name='form-manage-poli']",function(e){
		e.preventDefault();
		$("#save-poli").attr('disabled', 'disabled');
		$form=$(this).closest('form');
		var data = $form.serialize();
		http_request('master-data/poli/save__','POST',data)
		.done(function(res){
			$($modalManagePoli).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-poli").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-poli").removeAttr('disabled');
			})
	...
	.on("click",".link-delete-poli",function(){
		hideMsg();
		var id=$(this).data('id');
		$that=$(this);
		bootbox.confirm({
			...
			callback: function (result) {
				if(result){
					http_request('master-data/poli/delete__/'+id,'DELETE',{})
					.done(function(res){
							Msg.success(res.message);
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				...
				$.ajax({
					url: base_url('master-data/poli/import_'),
					type: 'POST',
					data: formData,
					async: false,
					cache: false,
					...
					success: function (response) {
						...
						if (response.duplicate && response.duplicate.length>0) {
							$(".row-error .content").html('');
							$(".row-error .title").html('Polilinik sudah terdaftar (Duplicate):');
