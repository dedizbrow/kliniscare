$modalManageSupplier = "#modal-manage-supplier";
$modalManageImport = "#modal-manage-import";
$tableData = $("#dataSupplier").DataTable({
    serverSide: true,
    ordering: true,
    pageLength: "50",
    ajax: {
        url: base_url('master-data/supplier/load-dt'),
        type: 'POST',
        headers: {
    ...
    columnDefs: [
		...
		{ targets: [1,4], width: '110px',className: 'text-left' },
...
$(document)
	.ready(function () {
		getActiveLang('master-data/supplier');
	})
	.on("click", ".add-supplier", function () {
		$modal_id=$modalManageSupplier;
	...
	.on("click",".link-edit-supplier",function(){
		http_request('master-data/supplier/search__','GET',{id: $(this).data('id')})
	...
	.on("submit","form[name='form-manage-supplier']",function(e){
		e.preventDefault();
		$("#save-supplier").attr('disabled', 'disabled');
		$form=$(this).closest('form');
		var data = $form.serialize();
		http_request('master-data/supplier/save__','POST',data)
		.done(function(res){
			$($modalManageSupplier).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-supplier").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-supplier").removeAttr('disabled');
			})
	...
	.on("click",".link-delete-supplier",function(){
		hideMsg();
		var id=$(this).data('id');
		$that=$(this);
		bootbox.confirm({
			...
			callback: function (result) {
						if(result){
							http_request('master-data/supplier/delete__/'+id,'DELETE',{})
							.done(function(res){
									Msg.success(res.message);
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				...
				$.ajax({
					url: base_url('master-data/supplier/import_'),
					type: 'POST',
					data: formData,
					async: false,
					cache: false,
					...
					success: function (response) {
						...
						if (response.duplicate && response.duplicate.length>0) {
							$(".row-error .content").html('');
							$(".row-error .title").html('Supplier sudah terdaftar (Duplicate):');
