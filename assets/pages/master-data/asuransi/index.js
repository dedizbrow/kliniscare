$modalManageAsuransi = "#modal-manage-asuransi";
$modalManageImport = "#modal-manage-import";
$tableData = $("#dataAsuransi").DataTable({
    serverSide: true,
    ordering: true,
    pageLength: "50",
    ajax: {
        url: base_url('master-data/asuransi/load-dt'),
        type: 'POST',
        headers: {
...
$(document)
	.ready(function () {
		getActiveLang('master-data/asuransi');
	})
	.on("click", ".add-asuransi", function () {
		$modal_id=$modalManageAsuransi;
	...
	.on("click",".link-edit-asuransi",function(){
		http_request('master-data/asuransi/search__','GET',{id: $(this).data('id')})
	...
	.on("submit","form[name='form-manage-asuransi']",function(e){
		e.preventDefault();
		$("#save-asuransi").attr('disabled', 'disabled');
		$form=$(this).closest('form');
		var data = $form.serialize();
		http_request('master-data/asuransi/save__','POST',data)
		.done(function(res){
			$($modalManageAsuransi).modal('hide');
			$tableData.ajax.reload();
			Msg.success(res.message);
			$("#save-asuransi").removeAttr('disabled');
		})
			.fail(function () {
				$("#save-asuransi").removeAttr('disabled');
			})
	...
	.on("click",".link-delete-asuransi",function(){
		hideMsg();
		var id=$(this).data('id');
		$that=$(this);
		bootbox.confirm({
			...
			callback: function (result) {
						if(result){
							http_request('master-data/asuransi/delete__/'+id,'DELETE',{})
							.done(function(res){
									Msg.success(res.message);
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				...
				$.ajax({
					url: base_url('master-data/asuransi/import_'),
					type: 'POST',
					data: formData,
					async: false,
					cache: false,
					...
					success: function (response) {
						...
						if (response.duplicate && response.duplicate.length>0) {
							$(".row-error .content").html('');
							$(".row-error .title").html('Asuransi sudah terdaftar (Duplicate):');
