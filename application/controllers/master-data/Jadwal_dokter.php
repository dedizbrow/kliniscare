...
class Jadwal_dokter extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->lang->load('master-data/jadwal_dokter', $this->session->userdata('site_lang'));
		$this->load->model("master-data/Jadwal_dokter_model", "j");
		$this->load->helper('Authentication');
		$this->data = isAuthorized();
		isAllowed('c-jadwaldokter');
	}

	public function index()
	{
		$this->data["web_title"] = lang('app_name_short') . " | Jadwal dokter";
		$this->data["page_title"] = "Jadwal dokter";
		$this->data['js_control'] = "master-data/jadwal_dokter/index.js";
		$this->data['datatable'] = true;
		$this->data['chartjs'] = false;

		$this->template->load(get_template(), 'master-data/jadwal_dokter/index', $this->data);
	}
	...
	public function search__($id = '')
	{
		...
		$search = $this->j->_search(array("idJadwal" => $id));
	...
	public function select2_()
	{
		...
		$search = $this->j->_search_select2($key, $gets['clinic_id']);
		echo json_encode($search);
	}
	public function select2_hari()
	{
		header('Content-Type: application/json');
		$gets = $this->input->get();
		$key = (isset($gets['search']) && $gets['search'] != '') ? $gets['search'] : "";
		$search = $this->j->_search_select_hari($key);
		echo json_encode($search);
	}
	public function save__()
	{
		...
		if ($dokter == "") return sendError("Pilih dokter");
		if ($hari == "") return sendError("pilih hari ");
		if ($dari_jam == "") return sendError("pilih waktu mulai ");
		if (!isset($posted['_id']) || $posted['_id'] == "") {
			isAllowed("c-jadwaldokter^create");
			$save = $this->j->_save(array(
				"dokter" 	=> $dokter,
				"clinic_id" => $clinic_id,
				"hari" 		=> $hari,
				"dari_jam" 	=> $dari_jam,
			...
			), array(), "dokter", "hari", "dari_jam", "clinic_id");
			if ($save == 'exist') {
			...
			isAllowed("c-jadwaldokter^update");
			$idJadwal = htmlentities(trim($posted['_id']));
			$data = array(
				"dokter" 	=> $dokter,
				"clinic_id" => $clinic_id,
				"hari" 		=> $hari,
			...
			$save = $this->j->_save($data, $where, "dokter", "hari", "dari_jam", "clinic_id");
			if ($save === "exist") {
	...
	{
		header('Content-Type: application/json');
		isAllowed("c-jadwaldokter^delete");
		$method = $this->input->method(true);
		if ($method != "DELETE") sendError(lang('msg_method_delete_required'), [], 405);
		$result = $this->j->_delete(array('idJadwal' => htmlentities(trim($id))));
		if ($result == 1) {
	...
	public function export_()
	{
		...
		$allUsers = $this->j->jadwal_list($clinic_id);
		$heading = array('No. ', 'Nama Dokter', 'Hari', 'dari_jam', 'sampai_jam');
		$sheet->getActiveSheet()->setTitle('Jadwal Dokter');
		$from = "A1";
		$to = "E1";
		...
		foreach ($allUsers as $user) {
			...
			$sheet->getActiveSheet()->setCellValue('C' . $row, $user->namaHari);
			$sheet->getActiveSheet()->setCellValue('D' . $row, $user->dari_jam);
			$sheet->getActiveSheet()->setCellValue('E' . $row, $user->sampai_jam);
			$row++;
			$no++;
		}
		$styleBorder = array(
	...
	public function import_()
	{
		...
		$uploaded = (object) single_upload($paths, "file", "import_data_jadwal__" . date("Ymdhis") . "-" . $rand, ["xls", "xlsx"]);
		$upd_file = $paths . "/" . $uploaded->file_name;
		$ext_file = $uploaded->file_ext;
		$inputFileName = $upd_file;

		$list_dokter = $this->j->_list_dokter();

		$arr_dokter = [];

		foreach ($list_dokter as $item) {
			$arr_dokter[$item->namaDokter] = $item;
		}

		$list_hari = $this->j->_list_hari();
		$arr_hari = [];
		foreach ($list_hari as $items) {
			$arr_hari[$items->namaHari] = $items;
		}
		$clinic_id = (!isset($clinic_id) || $clinic_id == 'default') ? getClinic()->id : $clinic_id;
		if ($clinic_id == 'allclinic') return sendError("Klinik Belum di pilih");
		...
		try {
			...
			if (!empty($rows_error)) {
				...
				foreach ($rows as $item) {
					$dta = $item;
					unset($dta['row_no']);
					$save = $this->j->_save($dta, array(), "dokter", "hari", "dari_jam", "clinic_id");
					if ($save == 'exist') {
						array_push($failed, "Row " . $item['row_no'] . " | " . $dta['dokter'] . " | " . $dta['hari']);
					} else {
						$success++;
					}
				}
