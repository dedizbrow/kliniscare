...
class Diagnosis extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->lang->load('master-data/diagnosis', $this->session->userdata('site_lang'));
		$this->load->model("master-data/Diagnosis_model", "d");
		$this->load->helper('Authentication');
		$this->data = isAuthorized();
		isAllowed('c-diagnose');
	}

	public function index()
	{
		$this->data["web_title"] = lang('app_name_short') . " | Diagnosa";
		$this->data["page_title"] = "Diagnosa";
		$this->data['js_control'] = "master-data/diagnosis/index.js";
		$this->data['datatable'] = true;
		$this->data['chartjs'] = false;

		$this->template->load(get_template(), 'master-data/diagnosis/index', $this->data);
	}
	...
	public function search__($id = '')
	{
		...
		$search = $this->d->_search(array("idDiagnosis" => $id));
	...
	public function save__()
	{
		...
		if ($kodeDiagnosis == "") return sendError("Kode Diagnosis wajib diisi");
		if ($namaDiagnosis == "") return sendError("Nama diagnosis Wajib diisi");
		if (!isset($posted['_id']) || $posted['_id'] == "") {
			isAllowed("c-diagnose^create");
			$save = $this->d->_save(array(
				"kodeDiagnosis" => $kodeDiagnosis,
				"clinic_id" 	=> $clinic_id,
				"namaDiagnosis" => $namaDiagnosis,
				"deskripsi" 	=> $deskripsi,
				'creator_id'    => $this->data['C_UID']
			), array(), "kodeDiagnosis", "clinic_id");
			if ($save == 'exist') {
			...
			isAllowed("c-diagnose^update");
			$id_diagnosis = htmlentities(trim($posted['_id']));
			$data = array(
				"kodeDiagnosis" => $kodeDiagnosis,
				"clinic_id" 	=> $clinic_id,
				"namaDiagnosis" => $namaDiagnosis,
			...
			$where = ["idDiagnosis" => $id_diagnosis];
			$save = $this->d->_save($data, $where, "kodeDiagnosis", "clinic_id");
			if ($save === "exist") {
	...
	{
		header('Content-Type: application/json');
		isAllowed("c-diagnose^delete");
		$method = $this->input->method(true);
		if ($method != "DELETE") sendError(lang('msg_method_delete_required'), [], 405);
		$result = $this->d->_delete(array('idDiagnosis' => htmlentities(trim($id))));
		if ($result == 1) {
	...
	public function export_()
	{
		...
		$allUsers = $this->d->diagnosislist($clinic_id);
		$heading = array('No. ', 'Kode Diagnosa', 'Nama Diagnosa', 'Deskripsi');
		$sheet->getActiveSheet()->setTitle('Diagnosa');
		$from = "A1";
		$to = "D1";
		...
		foreach ($allUsers as $user) {
			...
			$sheet->getActiveSheet()->setCellValue('B' . $row, $user->kodeDiagnosis);
			$sheet->getActiveSheet()->setCellValue('C' . $row, $user->namaDiagnosis);
			$sheet->getActiveSheet()->setCellValue('D' . $row, $user->deskripsi);
			$row++;
			$no++;
		}
		$styleBorder = array(
	...
	public function import_()
	{
		...
		$uploaded = (object) single_upload($paths, "file", "import_data_diagnosa__" . date("Ymdhis") . "-" . $rand, ["xls", "xlsx"]);
		...
		try {
			...
			if (!empty($rows_error)) {
				...
				foreach ($rows as $item) {
					$dta = $item;
					unset($dta['row_no']);
					$save = $this->d->_save($dta, array(), "kodeDiagnosis", "clinic_id");
					if ($save == 'exist') {
						array_push($failed, "Row " . $item['row_no'] . " | " . $dta['kodeDiagnosis'] . " | " . $dta['namaDiagnosis']);
					} else {
						$success++;
					}
				}
