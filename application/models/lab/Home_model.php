...
class Home_model extends CI_Model
{
	function __construct()
	{
		...
		$this->tableJenisSampling = "kok gk ke RS wae_sampling";
		$this->tableOpsiHasil = "lab__opsi_hasil";
	...
	function _load_dt_jumlah($posted, $provider_id = '')
	{
		...
		$limit = 0;
		$offset = 25;
		$data = $this->db->query("SELECT provider.nama as nama_provider,COUNT(pasien.id_pasien) as jumlah FROM tbl_pasien pasien INNER JOIN lab_data_provider provider ON pasien.provider_id=provider.id $sWhere $sGroup $sOrder")->result_object();
